...
| 3 (toggle UI + RSVP dual-write) | `<TBD post-merge>` | `v-pre-pr-4-toggle-flip` (rollback target for PR 4 — the operational flip) | `<TBD post-deploy>` | `pre-pr-3-toggle` is the rollback target. PR 3's pre-merge testing exercises the per-PR Neon branch the integration auto-provisions for `feat/migration-3-toggle`. | **First Sheets snapshot** — make a date-stamped Drive copy of the spreadsheet before PR 3 merges. PR 3 introduces the dual-write path, so any RSVP submitted while `writeMode='dual'` will hit Sheets too. |
...
- **PR 3 — Toggle UI + RSVP dual-write** *(this PR)* — Re-enables Settings tab, adds Data source / Write mode radios + `setDataSource`/`setWriteMode` server actions. Rewrites `api/rsvp/route.ts` per C4 (DB-first dual-write, fail-fast on DB error, log-and-continue on Sheets error in dual mode). Wires `revalidatePublicData()` into all 21 admin server actions. Seeds `Setting` rows: `(public, dataSource, NULL, 'sheets')` and `(public, writeMode, NULL, 'dual')`.
