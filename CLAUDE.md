...
│   ├── linkedPlayers.ts              # /assign-player picker: pure annotator + Prisma `Player.lineId` fetch (PR 14) — greys out players already linked to another LINE user
...
| 14 (filter already-linked players from /assign-player picker — Prisma `Player.lineId` query + greyed-out non-clickable rows) | `e583e0b` | `v-pre-pr-15-next` (rollback target for PR 15) | `https://t9l-website-2c997l658-t9l-app.vercel.app` | N/A — PR 14 is code-only, no schema change | N/A — PR 14 doesn't touch Sheets |
| 14.5 (chore — backfill PR 14 ledger row) | TBD (filled post-merge) | (no rollback tag — docs-only) | unchanged from PR 14 | N/A | N/A |
