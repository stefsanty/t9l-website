...
> **Current release:** v1.3.0 — Second in the cold-lambda perceived-latency push on assign-player (PR 11). Drops the awaited `update()` between API success and navigation in `AssignPlayerClient.handleConfirm`/`handleUnassign`. Pre-fix, the post-API chain serialized API write → `await update()` → `postAssignNavigate`. `update()` triggers a `/api/auth/session` request that runs the JWT callback server-side; while PR 8/9 brought warm-cache p50 to 108ms, this site runs cold-most-of-the-time (low traffic, low repeat-session rate) so most assigns paid a *separate* cold-lambda spin-up of 1–3s on top of the API write — that round-trip lived squarely on the user-visible critical path. v1.3.0 makes the refresh fire-and-forget (`kickOffSessionRefresh` helper, `void update().catch(...)`): same lambda spin-up, but parallel to navigation rather than serial, fully removed from the perceived path. Brief stale-flash on the destination is acceptable (next-auth's local session cache catches up within ~1s and re-renders the affected client components — Dashboard "Playing as" badge, RSVP bar, etc.). Helper lives in `src/lib/kickOffSessionRefresh.ts`; tests/unit/kickOffSessionRefresh.test.ts pins three contracts: update is called exactly once, the helper returns synchronously without awaiting the Promise (regression target — re-introducing `await` fails this test), and rejections are swallowed via `console.warn`. The Playwright timing assertion in `tests/e2e/assign-player.spec.ts` was tightened from 1000ms to 200ms (Definition of Done) — with PR 11 the button-label flip is strictly the React re-render after API response and trivially fits 200ms in local dev. Minor bump (1.2.7 → 1.3.0) because the auth-flow semantics change (session refresh is now best-effort instead of synchronous); rollback is a one-line revert (re-add `await`). PR 10 (v1.2.7) shipped the redundant-`router.refresh()` removal + `startTransition` wrap before this; together they remove two cold-startable round-trips from the assign-player critical path. Constant lives in `src/lib/version.ts`; bump there and update this line on each release.
...
│   ├── postAssignNavigate.ts         # Pure helper for post-API navigation (PR 10) — push '/' inside startTransition, no refresh
│   ├── kickOffSessionRefresh.ts      # Fire-and-forget next-auth session refresh (PR 11) — non-awaited; off the user-visible critical path
...
| 11 (drop awaited `update()` — fire-and-forget session refresh off the critical path) | TBD (filled post-merge) | (rollback target for PR 12 — created at PR 12's merge) | TBD (filled post-merge) | N/A — PR 11 is code-only, no schema change | N/A — PR 11 doesn't touch Sheets |
