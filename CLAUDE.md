...
> **Current release:** v1.2.2 — Sheets→DB cutover complete; LINE-player mapping migrated from Upstash Redis to Prisma `Player.lineId` (B1 cutover); new `LineLogin` Prisma model upserted on every LINE OAuth callback; admin "Assign Player" Flow B in the league-context Players tab links orphan LINE logins to Player records via `adminLinkLineToPlayer` server action; legacy Redis `line-player-map` reads remain as a fallback for the deprecation window (every hit logs `[auth] DEPRECATED Redis hit`). **Public self-assign UX fix v2: button leaves "Saving…" the moment the API write succeeds (was: stayed "Saving…" for the full 5–7s navigation chain because the loading flag spanned API + next-auth update + push + destination RSC render). New `redirecting` state + state-machine helper in `src/lib/assignButtonLabel.ts` with vitest coverage.** Version label visible on public + admin shells and inside the public-site LINE-user dropdown. Constant lives in `src/lib/version.ts`; bump there and update this line on each release.
...
| 7 (saving-stuck UX fix v2 — `redirecting` state + state-machine helper) | _pending merge_ | `v-pre-pr-8-...` (next PR's rollback target) | _pending Vercel deploy_ | N/A — PR 7 is UI-only, no schema change | N/A — PR 7 doesn't touch Sheets |
