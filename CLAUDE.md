...
| 14 (filter already-linked players from /assign-player picker — Prisma `Player.lineId` query + greyed-out non-clickable rows) | TBD (filled post-merge) | `v-pre-pr-15-next` (rollback target for PR 15 — created at PR 15's merge) | TBD (filled post-merge) | N/A — PR 14 is code-only, no schema change | N/A — PR 14 doesn't touch Sheets |
