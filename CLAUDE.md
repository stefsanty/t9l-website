...
> **Current release:** v1.2.6 — Docs-only follow-up to v1.2.5: backfills the PR 9 ledger row (merge SHA `f9f2db3`, prod deploy `https://t9l-website-q01pe7pnv-t9l-app.vercel.app`) and flips PR 9's migration-status entry from "pending merge" to "shipped". v1.2.5 itself closes the post-write hole left by v1.2.3 (PR 8). PR 8's mapping cache busted on every write so the next JWT callback paid the cold-Neon Prisma `findUnique` cost — exactly the read that runs inside `await update()` on the assign-player flow, where the user feels it as a 1–3s tail on top of the API write. v1.2.5 (PR 9) flips writes from `invalidate(lineId)` to `setCached(lineId, freshMapping)` at every site that mutates `Player.lineId` (`api/assign-player` POST sets the post-write `{playerId, playerName, teamId}` shape, DELETE sets the null sentinel; `admin/actions.ts#updatePlayer` keeps invalidate for the OLD lineId but pre-warms the NEW; `admin/actions.ts#createPlayer` and `admin/leagues/actions.ts#adminLinkLineToPlayer` pre-warm via the now-exported `lib/auth.ts#getPlayerMappingFromDb` for shape parity with the JWT path). The post-assign `await update()` call now hits cache and returns within the v1.2.3 warm-cache envelope (median 108ms / max 379ms in the v1.2.3 A/B), instead of cold-pathing to 1–3s. Vitest pins the regression-prevention contract (`tests/unit/assignPlayerCachePrewarm.test.ts` — POST calls setCached with the slug-only mapping, DELETE calls setCached with null, neither calls invalidate). Constant lives in `src/lib/version.ts`; bump there and update this line on each release.
...
| 9 (post-write pre-warm — closes the v1.2.3 invalidate-then-cold hole) | `f9f2db3` | (rollback target for PR 10 — to be created at next PR's merge) | `https://t9l-website-q01pe7pnv-t9l-app.vercel.app` | N/A — PR 9 is code-only, no schema change | N/A — PR 9 doesn't touch Sheets |
...
- **PR 10 — Retire Sheets path + remove Redis line-player-map fallback** — Pending; only after weeks of confidence operating on `dataSource='db'` AND zero `[auth] DEPRECATED Redis hit` log entries. Will switch `writeMode` to `'db-only'`, delete the legacy Sheets reads, and remove `getPlayerMappingFromRedis` from `lib/auth.ts`.
