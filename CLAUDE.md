...
> **Current release:** v1.2.7 — First in a multi-PR push to bring the assign-player flow's *cold-lambda* perceived latency down (this site runs cold-most-of-the-time, so warm-cache wins from PRs 8/9 don't represent the user's reality — recent feedback puts the click-to-`/`-rendered transition at 5–7s end-to-end). PR 10 drops the redundant `router.refresh()` after `router.push('/')` in `AssignPlayerClient` and wraps the navigation in `startTransition`. Pre-fix the post-API path fired *two* RSC fetches: one for the navigation, one to invalidate the client cache. The client-cache invalidation was already covered by the API route's `revalidatePath('/') + revalidateTag('public-data', { expire: 0 })` (Next propagates server-side revalidations to the client router cache on next navigation), so the extra `refresh()` was wasted work — and under the cold-lambda steady-state regime that's a full 1–3s critical-path round-trip. Navigation logic extracted to `src/lib/postAssignNavigate.ts` so the call shape (push exactly once, refresh never, both inside a transition) is unit-testable in `tests/unit/postAssignNavigate.test.ts` without React Testing Library. v1.2.6 was a docs-only follow-up to v1.2.5 (backfilled PR 9 ledger row); v1.2.5 itself closed the post-write hole left by v1.2.3 (PR 8) — PR 8's mapping cache busted on every write so the next JWT callback paid the cold-Neon Prisma `findUnique` cost — exactly the read that runs inside `await update()` on the assign-player flow. v1.2.5 (PR 9) flipped writes from `invalidate(lineId)` to `setCached(lineId, freshMapping)` at every site that mutates `Player.lineId` (`api/assign-player` POST/DELETE, `admin/actions.ts#updatePlayer`/`createPlayer`, `admin/leagues/actions.ts#adminLinkLineToPlayer`). The post-assign `await update()` call hits cache and returns within the v1.2.3 warm-cache envelope (median 108ms / max 379ms in the v1.2.3 A/B), instead of cold-pathing to 1–3s — but only when the cache is warm; the next PR (β) drops `await update()` entirely so the navigation no longer waits on a separately-cold-startable `/api/auth/session` round-trip. Constant lives in `src/lib/version.ts`; bump there and update this line on each release.
...
| 10 (drop redundant `router.refresh()` + wrap navigation in `startTransition`) | TBD (filled post-merge) | (rollback target for PR 11 — created at PR 11's merge) | TBD (filled post-merge) | N/A — PR 10 is code-only, no schema change | N/A — PR 10 doesn't touch Sheets |
...
- **PR 10 — Drop redundant `router.refresh()` + wrap navigation in `startTransition`** ✅ shipped (TBD merge SHA, 2026-04-28). First in a multi-PR push targeting cold-lambda perceived latency on assign-player. Removes the second RSC fetch that was firing after every assign/unassign because the API route already calls `revalidatePath('/') + revalidateTag('public-data', { expire: 0 })` server-side; Next propagates that to the client router cache on next navigation, making `router.refresh()` strictly redundant. Under cold-lambda steady-state that redundant fetch was a 1–3s critical-path round-trip. Navigation extracted to `src/lib/postAssignNavigate.ts`; Vitest pins the call shape (push exactly once with `'/'`, `refresh` never, both inside the transition callback) in `tests/unit/postAssignNavigate.test.ts`. No schema change. PR β (next) drops `await update()` to remove another cold-startable round-trip.
- **PR 11+ — Retire Sheets path + remove Redis line-player-map fallback** — Pending; only after weeks of confidence operating on `dataSource='db'` AND zero `[auth] DEPRECATED Redis hit` log entries. Will switch `writeMode` to `'db-only'`, delete the legacy Sheets reads, and remove `getPlayerMappingFromRedis` from `lib/auth.ts`.
