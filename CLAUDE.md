...
- **PR 19+ — Retire Sheets path + write-mode flip** — Pending; only after weeks of confidence operating on `dataSource='db'`. Will switch `writeMode` to `'db-only'` and delete the legacy Sheets reads. (PR 16 already retired the legacy Redis line-player-map fallback that this entry previously bundled.)
