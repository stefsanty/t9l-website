...
> **Current release:** v1.2.4 — Docs-only follow-up to v1.2.3: backfills the PR 8 ledger row (merge SHA `96d4605`, prod deploy `https://t9l-website-gwukc7ljc-t9l-app.vercel.app`) and flips PR 8's migration-status entry from "pending merge" to "shipped". v1.2.3 itself adds an Upstash Redis cache-aside layer in front of the LINE-ID → Player mapping read that runs on every JWT callback. Pre-cutover this read was a 50ms `HGET line-player-map`; PR 6 made it a Prisma `findUnique` with a relation include, which on cold Neon serverless connections regressed `/api/auth/session` p50 from ~50ms to a 250ms–1.4s envelope. New helper `src/lib/playerMappingCache.ts` (60s TTL, `t9l:auth:map:` namespace, null sentinel for unmapped IDs) is wired into `lib/auth.ts#getPlayerMapping`; cache is busted on every Prisma `Player.lineId` write (`api/assign-player` POST + DELETE, admin `updatePlayer`/`createPlayer`/`adminLinkLineToPlayer`). Local A/B against the dev preview Neon branch: median `/api/auth/session` 264ms → 108ms; max 1369ms → 379ms (~60% reduction at p50, ~72% at tail). Version label visible on public + admin shells and inside the public-site LINE-user dropdown. Constant lives in `src/lib/version.ts`; bump there and update this line on each release.
...
| 8 (Prisma-on-every-JWT perf fix — Upstash mapping cache) | `96d4605` | `v-pre-pr-9-jwt-cache` (rollback target for PR 9 — to be created at next PR's merge) | `https://t9l-website-gwukc7ljc-t9l-app.vercel.app` | N/A — PR 8 is code-only, no schema change | N/A — PR 8 doesn't touch Sheets |
