...
import CopyMatchdayLink from './CopyMatchdayLink';
    ...
    <div className={`pl-card rounded-3xl overflow-hidden relative group transition-all duration-500 ${
      ...
      <div className="p-5 pb-4 relative">
        <div className={`transition-opacity duration-500 ${isSittingOut ? 'opacity-40' : ''}`}>
          <div className="mb-2">
            <div className="flex items-center gap-2 mb-1">
              ...
              {/* v1.48.0 — eyebrow is now click-to-copy: copies the
                  deep-link URL `https://<host>/matchday/<id>` to clipboard
        ...
        <div className={`transition-opacity duration-500 ${isSittingOut ? 'opacity-40' : ''}`}>
          ...
          {/* v1.48.0 — "View matchday" link removed. The homepage IS the
              matchday page (Dashboard accepts initialMatchdayId; the per-
              matchday route renders the same Dashboard with that matchday
              pre-selected). The eyebrow above is now click-to-copy for
              sharing the matchday-specific URL. */}
