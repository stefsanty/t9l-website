...
export default function TeamsTab({ leagueId, leagueTeams, allTeams }: TeamsTabProps) {
  ...
  const record = selected ? computeRecord(selected) : null

  return (
    <div className="flex gap-0 rounded-xl border border-admin-border overflow-hidden bg-admin-surface">
          ...
          {showEnroll ? (
            <div className="flex flex-col gap-2">
              <select
                ...
                className="bg-admin-surface3 border border-admin-border text-admin-text text-xs rounded px-2 py-1.5 w-full"
              >
              ...
              <div className="flex gap-1.5">
                <button
                  ...
                  className="flex-1 px-2 py-1 bg-admin-green text-admin-bg text-xs font-medium rounded hover:opacity-90 disabled:opacity-50"
                >
                  Enroll
                </button>
                <button
                  onClick={() => { setShowEnroll(false); setEnrollTeamId('') }}
                  className="px-2 py-1 border border-admin-border text-admin-text2 text-xs rounded hover:border-admin-border2"
                >
                  Cancel
                </button>
              </div>
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-admin-text3 text-xs hover:text-admin-green transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
        ...
        {!selected ? (
          ...
          <div>
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                <h3 className="font-condensed font-bold text-admin-text text-2xl">{selected.team.name}</h3>
                <p className="text-admin-text3 text-sm mt-0.5">
                  {selected.playerAssignments.length} player{selected.playerAssignments.length !== 1 ? 's' : ''}
                </p>
              </div>
              <ConfirmDialog
                trigger={
                  <button className="flex items-center gap-1.5 text-admin-text3 text-xs hover:text-admin-red transition-colors border border-admin-border rounded px-3 py-1.5 hover:border-admin-red/30">
                    <X className="w-3 h-3" />
                    Remove from league
                  </button>
                }
                title={`Remove ${selected.team.name}?`}
                description="This will unenroll the team from this league. Players will lose their assignments."
                confirmLabel={`Remove ${selected.team.name}`}
                onConfirm={() => handleRemove(selected.id, selected.team.name)}
              />
            </div>
            ...
            <div>
              ...
              {selected.playerAssignments.length === 0 ? (
                <p className="text-admin-text3 text-sm">No players assigned to this team.</p>
              ) : (
                <div className="flex flex-col gap-1">
                  {selected.playerAssignments.map((pa) => (
                    <div
                      key={pa.id}
                      className="flex items-center px-3 py-2 rounded-lg bg-admin-surface2 text-sm text-admin-text"
                    >
                      {pa.player.name}
                    </div>
                  ))}
