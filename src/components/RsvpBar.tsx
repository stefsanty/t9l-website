  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-lg z-40">
        <div className={`px-4 pt-3 pb-5 border-t-2 transition-colors ${isGoing && !showOptions ? 'bg-[#00E87A] border-[#00C066]' : 'bg-background border-primary/60'}`}>
            ...
            <div>
              <div className="flex items-center justify-between gap-3 mb-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-sm font-black text-black/70 uppercase tracking-wide truncate">{matchday.label}</span>
                </div>
                <button
                  onClick={() => setShowOptions(true)}
                  className="text-xs font-bold text-black/60 underline underline-offset-2 decoration-black/30 hover:text-black transition-colors shrink-0 whitespace-nowrap"
                >
                  edit
                </button>
              </div>
