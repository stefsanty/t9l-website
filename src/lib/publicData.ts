 ...
 *
 * Pre-v1.71.0 this file held a dispatcher between the legacy Google-Sheets
 * parser and the Postgres reader. v1.71.0 retires the Sheets path; only the
 * DB reader remains.
 *
 * v1.7.0 — RSVP signals (availability / availabilityStatuses / played) are
 * sourced from Redis at dispatch time and merged into the same `LeagueData`
 * shape. RSVP writes are read-your-own-writes consistent without a
 * cache-bust round-trip.
 *
 * v1.14.0 — `getFromDb` returns `{ data, gameWeeks }`. The DB-side
...
/**
 * Returns the public `LeagueData` from Postgres + merged Redis RSVP.
 *
 ...
 * league flagged `isDefault: true` — preserves pre-v1.23.0 behavior.
 *
 * v1.7.0 — The cached static blob and the live Redis RSVP read run in
 * parallel; merge produces the consumer-facing shape.
 *
 * v1.71.0 — Sheets dispatcher removed; this is the only path.
 */
 ...
 *
 * v1.8.2 — Routes the route's player-existence check through this helper
 * so link / unlink avoid the RSVP fanout on every call.
 */
export async function getPlayerByPublicId(
  publicPlayerId: string,
  leagueId?: string,
): Promise<{ id: string; name: string; teamId: string } | null> {
  const players = (await getFromDb(leagueId)).data.players
